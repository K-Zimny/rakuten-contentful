        ...
        <div className="my-8 rounded-2xl border border-gray300 shadow-md bg-gray-50 p-6">
            ...
            {(updatedProject.skillsUsed && updatedProject.skillsUsed.length > 0) && (
                <div className="mb-4" {...inspectorProps({fieldId: 'skillsUsed'})}>
                    <h3 className="mb-2 text-sm font-semibold text-gray-700">Skills Used:</h3>
                    <div className="flex flex-wrap gap-2">
                        {updatedProject.skillsUsed.map((skill, index) => (
                            skill && (
                                <span 
                                    key={index}
                                    className="rounded-full bg-blue-100 px-3 py-1 first:pl-0 text-sm text-blue-800"
                                >
                                    {skill}
                                </span>
                            )
            ...
            {(updatedProject.technologiesUsed && updatedProject.technologiesUsed.length > 0) && (
                <div className="mb-4" {...inspectorProps({fieldId: 'technologiesUsed'})}>
                    <h3 className="mb-2 text-sm font-semibold text-gray-700">Technologies Used:</h3>
                    <div className="flex flex-wrap gap-2">
                        {updatedProject.technologiesUsed.map((tech, index) => (
                            tech && (
                                <span 
                                    key={index}
                                    className="rounded-full bg-green-100 px-3 py-1 first:pl-0 text-sm text-green-800"
                                >
                                    {tech}
                                </span>
                            )
                ...
                <div className="border-t border-gray-200 pt-4">
                    ...
                    <div
                        className={`overflow-hidden transition-all duration-300 ease-in-out ${
                            isDetailsOpen ? 'max-h-[10000px] opacity-100' : 'max-h-0 opacity-0'
                        }`}
                    >
